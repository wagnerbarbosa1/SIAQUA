...
pipeline = Chronos2Pipeline.from_pretrained(
    "amazon/chronos-2",
    device_map="cuda",
    dtype=torch.float32 #é um tipo bom para IA porque representa grandes escalas sem usar tanta memoria quanto float32
)
...
for i, (trainidxs, testidxs) in enumerate(expanding_window_test.split()):
    ...
    context_future = y_t.drop(["water_produced"], axis=1) 
    ...
    for j in range(expanding_window_test.step):
        resultsEW_val["ytrue0"].append(ytrue.iloc[j])
        resultsEW_val["ytrue1"].append(ytrue.iloc[j+expanding_window_test.step])
        resultsEW_val["yhat0"].append(median[j])
        resultsEW_val["yhat1"].append(median[j+expanding_window_test.step])
        resultsEW_val["timestamp"].append(y_t["timestamp"].iloc[j])
...
ax.plot(df_result["timestamp"], df_result["yhat0"], color="#2554b9", linewidth=1.3, alpha=0.6, label='Chronos2')
